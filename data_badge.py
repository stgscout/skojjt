...
class BadgeImage(ndb.Model):
    ...
    isTemplate = ndb.BooleanProperty(required=True)  # Owned by template. Cannot be removed by others
...
class Badge(ndb.Model):
    ...
    @staticmethod
    def create(name, scoutgroup_key, description, parts_scout, parts_admin, img_data=None, img_key=''):
        ...
        if img_data:
            image = BadgeImage(image=img_data, name=name, isTemplate=False)
            badge.image_key = image.put()
        elif img_key:
            badge.image_key = img_key
        badge.put()
    ...
    def update(self, name, description, parts_scout, parts_admin, img_data):
        ...
        if img_data:
            ...
            if self.image_key is not None:
                image = self.image_key.get()
                if not image.isTemplate:
...
class BadgeTemplate(ndb.Model):
    ...
    @staticmethod
    def create(name, description, parts_scout, parts_admin, img_data=None):
        if name == "" or name is None:
            logging.error("No name set for new badge")
            return
        ...
        template = BadgeTemplate(name=name,
                                 ...
                                 parts_admin_long=parts_admin_long)
        if img_data:
            image = BadgeImage(image=img_data, name=name, isTemplate=True)
            template.image_key = image.put()
        template.put()
